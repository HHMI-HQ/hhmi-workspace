...
import { primitives, ui, RequestHelpDialog } from '@curvenote/scms-core';
 ...
 * - Uses database-stored flag to persist request state across sessions
 *
 */
export function ComplianceDashboardRequest({
  orcid,
  dashboardRequested = false,
}: OrcidNotFoundEmptyStateProps) {
  const [requestSent, setRequestSent] = useState<boolean>(dashboardRequested);
  const [showHelpDialog, setShowHelpDialog] = useState(false);

  // Sync state with prop when it changes (e.g., after page revalidation)
  useEffect(() => {
    setRequestSent(dashboardRequested);
  }, [dashboardRequested]);
  ...
  function handleRequestSuccess() {
    ...
    // The database flag is set by the action endpoint
    setRequestSent(true);
            ...
            <div className="p-6 space-y-6">
              ...
              {!requestSent && (
                <p className="text-sm text-center text-muted-foreground">
                  Once enabled, up to date reports on your compliance status will be available on
                  this page.
                </p>
              )}
            </div>
